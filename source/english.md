...
|98|Unavailable system/communication.|The request did not have a return until the timeout.|Your Transaction may have been processed. To verify if it was ok, please consult the transaction by your order number.|Retry only after check if the original transaction by order number and verify it was not processed.|
